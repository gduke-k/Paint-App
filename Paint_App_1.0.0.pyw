...
pen_brush_type=StringVar()
pen_brush_type.set("Pen")
brush_option=["Pen","Dotted","Boxed","Snow"]
brushx=IntVar()
...
def saveImage():
    try:
        ...
        messagebox.showinfo("Paint App: Not Save" , "Info: Not Saved")

def openImage():
    filename = filedialog.askopenfilename(title="Select Image", filetypes=[("Image Files", "*.jpg;*.jpeg;*.png")])
    photo_image = ImageTk.PhotoImage(Image.open(filename).resize((1020,660),Image.ANTIALIAS))
    canvas.image = photo_image
    canvas.delete('all')
    canvas.create_image(0, 0, image=photo_image, anchor='nw')

def clear():
    if messagebox.showinfo("Paint App Confirmation","Clearing workspace..."):
        canvas.delete('all')
...
def helpbar():
    ...
    helpwin.geometry("850x450+160+60")
    helpwin.title("Help")
    head_helpframe=Frame(helpwin,height=50,)
    head_helpframe.grid(row=0,column=0,sticky=NW,padx=350,pady=20)
    body_helpframe=Frame(helpwin,height=350)
    body_helpframe.grid(row=1,column=0,sticky=S,pady=20,padx=10)
    Label(head_helpframe, text="Paint Version 1.0.0"+'\n'+"2023 Work", font=('Helvetica 13 bold')).grid(row=0,column=0)
    Label(body_helpframe,wraplength=750, text="Paint app is a software application that allows users to create digital drawings, sketches or paintings using a computer.The app provides tools such as pencil, colors, and erasers to help users create their artwork.", font=('Helvetica 10')).grid(sticky=W,row=0,column=0)
    Label(body_helpframe,wraplength=750, text='\n'+"Some of Tools are:"+'\n'+" ", font=('Helvetica 10')).grid(sticky=W,row=1,column=0)
    Label(body_helpframe,wraplength=750, text="Pencil:Using pencil you can create your artwork on workspace", font=('Helvetica 10')).grid(sticky=W,row=2,column=0)
    Label(body_helpframe,wraplength=750, text="Eraser:Using Eraser you can Erase what is written by pencil", font=('Helvetica 10')).grid(sticky=W,row=3,column=0)
    Label(body_helpframe,wraplength=750, text="Default & Pen Size:Using pensize dropdown list you can select pen size. Default will make your pen size default which is 1", font=('Helvetica 10')).grid(sticky=W,row=4,column=0)
    Label(body_helpframe,wraplength=750, text="Brush:Using this button you can change brush type of pen.", font=('Helvetica 10')).grid(sticky=W,row=5,column=0)
    Label(body_helpframe,wraplength=750, text="Background Color:Your can use this button to change background color of workspace", font=('Helvetica 10')).grid(sticky=W,row=6,column=0)
    Label(body_helpframe,wraplength=750, text="New:You can create new workspace with this button", font=('Helvetica 10')).grid(sticky=W,row=7,column=0)
    Label(body_helpframe,wraplength=750, text="Open:You can Open Images.", font=('Helvetica 10')).grid(sticky=W,row=8,column=0)
    Label(body_helpframe,wraplength=750, text="Save:You can save your work from workspace into system storage using this.Default filetype is .jpg", font=('Helvetica 10')).grid(sticky=W,row=9,column=0)
    Label(body_helpframe,wraplength=750, text="Clear:This will clear your workspace", font=('Helvetica 10')).grid(sticky=W,row=10,column=0)
...
def setbrush(self):
    if pen_brush_type.get()==brush_option[0]:
        brushx.set(0)
    if pen_brush_type.get()==brush_option[1]:  
        brushx.set(1)
    if pen_brush_type.get()==brush_option[2]:  
        brushx.set(2)
    if pen_brush_type.get()==brush_option[3]:  
        brushx.set(3)
...
def paint(event):
    ...
    if preCondinate != [0,0] :
        ...
        if brushx.get()==1:
            canvas.create_arc(x,y,x+(pen_size.get())/2,y+(pen_size.get())/2,fill=pen_color.get(),outline=pen_color.get(),width=(pen_size.get())/2)
        if brushx.get()==2:
            canvas.create_rectangle(x,y,x+(pen_size.get())/2,y+(pen_size.get())/2,outline=pen_color.get(),fill=pen_color.get())
        if brushx.get()==3:
            if pen_size.get()==1:
                pen_size.set(2)
            canvas.create_line((x-(pen_size.get()/3),y-(pen_size.get()/3),x+pen_size.get()/3,y+pen_size.get()/3),fill=pen_color.get())
            canvas.create_line((x+(pen_size.get()/3),y-(pen_size.get()/3),x-pen_size.get()/3,y+pen_size.get()/3),fill=pen_color.get())
            canvas.create_line((x,y-(pen_size.get()/3),x,y+(pen_size.get()/3)),fill=pen_color.get())
            canvas.create_line((x-(pen_size.get()/3),y,x+(pen_size.get()/3),y),fill=pen_color.get())
...
colorframe=Frame(root,width=160,borderwidth=11,bg='grey96')
colorframe.grid(row=0,column=0,sticky=N,pady=30)
toolframe=Frame(root,width=160,bg='grey96')
toolframe.grid(row=0,column=0,sticky=S,pady=15)
...
brushlist=OptionMenu(toolframe, pen_brush_type, *brush_option,command=setbrush)
brushlist.config(height=2,width=12,highlightthickness=0,bg='grey75',relief=FLAT)
brushlist.grid(row=4,column=0,columnspan=2,pady=7,padx=0)

bodycolor=Button(toolframe,height=2,width=15,text="Backgroung Color",bg='grey75',relief=FLAT,command=backcolor)
bodycolor.grid(row=5,column=0,columnspan=2,pady=7)
newimage=Button(toolframe,height=2,width=15,text="New",bg='grey75',relief=FLAT,command=createnew)
newimage.grid(row=6,column=0,columnspan=2,pady=7)
saveimage=Button(toolframe,height=2,width=15,text="Save",bg='grey75',relief=FLAT,command=saveImage)
saveimage.grid(row=7,column=0,columnspan=2,pady=7)
clearimage=Button(toolframe,height=2,width=15,text="Clear",bg='grey75',relief=FLAT,command=clear)
clearimage.grid(row=8,column=0,columnspan=2,pady=7)
